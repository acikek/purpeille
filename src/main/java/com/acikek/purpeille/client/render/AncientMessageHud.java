...
public class AncientMessageHud implements HudRenderCallback {
    ...
    public static Identifier seriesId;
    ...
    public void onHudRender(DrawContext ctx, float tickDelta) {
        ...
        if (!ticks.isEmpty()) {
            ...
            List<? extends Text> lines = data.color == null
                    ? data.lines
                    : Arrays.stream(data.lines.get(0).getString().split(String.valueOf('\n')))
                            .map(Text::literal)
                            .toList();
            int color = data.color != null ? data.color : 0xFFFFFFFF;
            ...
            for (int i = 0; i < lines.size(); i++) {
                int textWidth = client.textRenderer.getWidth(lines.get(i));
                ctx.drawTextWithShadow(client.textRenderer, lines.get(i).asOrderedText(), (int) (-textWidth / 2.0f), (int) (i * (7 + client.textRenderer.fontHeight / 2.0f)), color);
            }
            matrixStack.pop();
        ...
        // Upon reaching the end of the series, send completed packet
        else if (seriesId != null) {
            PacketByteBuf buf = PacketByteBufs.create();
            buf.writeIdentifier(seriesId);
            ClientPlayNetworking.send(AncientMessages.getCompletedChannel(), buf);
            seriesId = null;
        }
    }
